...
interface VideoEmbedProps {
  onVideoAdd?: (videoData: VideoData) => void;
  className?: string;
}
...
const VideoEmbed = ({ onVideoAdd, className = "" }: VideoEmbedProps) => {
  ...
  const generateEmbedCode = (url: string, platform: string): string => {
    switch (platform) {
      case "tiktok":
        ...
        if (tiktokMatch) {
          return `<blockquote class="tiktok-embed" cite="${url}" data-video-id="${tiktokMatch[1]}" style="max-width: 605px;min-width: 325px;" > <section> <a target="_blank" title="@user" href="${url}">@user</a> </section> </blockquote> <script async src="https://www.tiktok.com/embed.js"></script>`;
        }
        break;
      case "instagram":
        // Instagram embed
  ...
  const handleAddVideo = () => {
    ...
    if (!platform) {
      alert(
        "URL tidak didukung. Gunakan URL dari TikTok, Instagram, atau Facebook.",
      );
      return;
    }
    ...
    const videoData: VideoData = {
      ...
      url: videoUrl,
      platform,
      embedCode: generateEmbedCode(videoUrl, platform),
    };
  ...
  const removeVideo = (id: string) => {
    setEmbeddedVideos((prev) => prev.filter((video) => video.id !== id));
  };

  const renderVideoEmbed = (video: VideoData) => {
    switch (video.platform) {
      case "tiktok":
        return (
          <div className="aspect-[9/16] max-w-[325px] mx-auto">
            <iframe
      ...
      case "facebook":
        return (
          <div className="max-w-[500px] mx-auto">
            ...
            >
              <p className="text-sm text-gray-600 mb-2">Video Facebook</p>
              <a
                href={video.url}
                target="_blank"
    ...
    <div className={`space-y-4 ${className}`}>
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="neumorphic-card border-0 shadow-none">
          <DialogHeader>
            <DialogTitle className="text-[#2d3748]">Embed Video</DialogTitle>
            <DialogDescription className="text-[#718096]">
              Masukkan URL video dari TikTok, Instagram, atau Facebook untuk
              menambahkannya ke postingan Anda.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="text-xs text-[#718096] space-y-1">
              <p>• TikTok: https://www.tiktok.com/@user/video/...</p>
              <p>• Instagram: https://www.instagram.com/p/...</p>
              <p>• Facebook: https://www.facebook.com/watch/...</p>
            </div>
          </div>
          <DialogFooter>
            <Button
      ...
      {embeddedVideos.length > 0 && (
        <div className="space-y-4">
          ...
          {embeddedVideos.map((video) => (
            <Card
              key={video.id}
              className="neumorphic-card border-0 shadow-none"
            >
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    <Video className="h-4 w-4 text-[#667eea]" />
                    <span className="text-sm font-medium text-[#2d3748] capitalize">
                      {video.platform}
                    </span>
                  </div>
                  <Button
